...
| Concurrency | `ReentrantLock`, `ConcurrentHashMap`, `CopyOnWriteArrayList`, `ScheduledExecutorService` |
...
### Kỹ thuật & Chất lượng mã

| # | Hạng mục | Trạng thái |
|---|---|---|
| 23 | OOP: Cây kế thừa `Entity → Item → Electronics/Art/Vehicle`, `Entity → User → Bidder/Seller/Admin` | ✅ Hoàn thành |
| 24 | Design Pattern: **Singleton** (`AuctionManager`), **Observer** (`AuctionObserver`/`ClientHandler`), **Factory** (tạo item theo loại) | ✅ Hoàn thành |
| 25 | Unit Test (JUnit 5): 7 test case — valid bid, bid thấp hơn, bid khi đóng phiên, concurrent bids, đóng không có bid (CANCELED), getWinner, handleGetAuctions JSON | ✅ Hoàn thành |
...
| Họ tên | MSSV | Đóng góp chính |
|---|---|---|
| *(Thành viên 1)* | *(MSSV)* | *(Phần đảm nhận)* |
| *(Thành viên 2)* | *(MSSV)* | *(Phần đảm nhận)* |
| *(Thành viên 3)* | *(MSSV)* | *(Phần đảm nhận)* |
| *(Thành viên 4)* | *(MSSV)* | *(Phần đảm nhận)* |
